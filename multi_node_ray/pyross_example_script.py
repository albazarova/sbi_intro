...
from sbi.inference import SNPE  # , SNRE, prepare_for_sbi
...
class CaseData:
    ...
    def to_pyross_oseir_array(
        self, total_population, time_exposed, time_infected
    ) -> np.array:
...
def simulator2(parameters):
    ...
    eps = 0.0001
    beta2 = np.max(
        (beta1 + np.array(parameters[1::3]), np.repeat(eps, beta1.shape[0])), axis=0
    )
    gE = 0.85  # recovery rate of asymptomatic infectives
    gI = np.max(
        (beta1 + np.array(parameters[2::3]), np.repeat(eps, beta1.shape[0])), axis=0
    ...
    def parameter_mapping(input_parameters, t):
        output_parameters = {"gE": input_parameters["gE"], "gI": input_parameters["gI"]}
        if t < 18:
    ...
    for ii in range(beta1.shape[0]):
        ...
        sto_model = pyross.stochastic.Model(
            model_spec, true_parameters, M, Ni, time_dep_param_mapping=parameter_mapping
        )
...
prior_gpu = utils.BoxUniform(
    low=torch.tensor([0.8, -0.3, -0.3], device=torch.device("cuda")),
    high=torch.tensor([1.5, 0.1, 0.1], device=torch.device("cuda")),
)

prior = utils.BoxUniform(
    low=torch.tensor([0.8, -0.3, -0.3]), high=torch.tensor([1.5, 0.1, 0.1])
)
...
inference = SNPE(prior_gpu, device="cuda")
...
if x != None:
    ...
    x1 = np.float32(x1)
    x1 = torch.from_numpy(x1).to("cuda")
    ...
    inference1 = SNPE(prior_gpu, device="cuda")
    x = np.float32(x)
    ...
    y_o = torch.from_numpy(y_o).to("cuda")
    # y_o=y_o.to('cuda')

    theta_p1 = posterior1.sample((10000,), x=y_o)

    observation1 = simulator2(
        torch.unsqueeze(torch.mean(theta_p1.detach().cpu(), axis=0), 0)
    )

    fig1, out1 = pairplot(
        theta_p1.detach().cpu().numpy(),
        limits=[[0.8, 1.5], [-0.3, 0.1], [-0.3, 0.1]],
        fig_size=(3, 3),
        upper="kde",
        diag="kde",
    ...
    (line2,) = ax.plot(
        casedata.data["cases_all_germany_ma7"].Confirmed[0:71], label="RKI data"
    )
